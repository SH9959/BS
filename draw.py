...
def d1(l1, l2,name,num):
    ...
    # 添加标题和标签
    plt.title(f'The statistical description of the layers on {num} samples')
    plt.xlabel('model')
    plt.ylabel('layer')

    # 保存图表
    plt.savefig(f'{name}_bar.png',dpi=1000)
...
def plot_histogram(data, name):
    ...
    plt.ylabel('times')
    plt.title('')
    plt.savefig(f'{name}_hist.png', dpi=1000)

def draw_line(labels:List[str], datas:List[List[Union[int, float]]]):
    """
    激活单元的数量折线图 3-4
    ...
    for i, d in enumerate(datas):
        plt.plot(d, marker='.' ,label=labels[i])
    plt.xlabel('Layer')
    plt.ylabel('Number of activated neurons')
    plt.xticks(range(len(datas[0])), range(0, len(datas[0]))) 
    plt.title('不同模型输入‘爱因斯坦的专业是’各个层的激活单元数量变化')
    # 添加图例
    plt.legend()
    # 显示图表
    plt.savefig('activated_line_chart.png', dpi=1000)


def show_info_in_gragh(info_list: Dict, one_tgt: str, prompt: str, layername: str ,model_name: str = "LlaMA3"):
    x = []
    y_rank = []
    y_prob = []
    y_delt = []
    ...
    ax2 = ax1.twinx()
    bar_width = 0.4  # 设置柱宽
    ax2.bar(x, y_prob, alpha=0.6, width=bar_width, edgecolor='black', linewidth=0.5, linestyle='--', hatch='/',
            label=f'prob of ‘{one_tgt}’')  # 柱形图
    # 绘制 y_delt，使用偏移量使其与y_prob对齐
    ax2.bar(x, y_delt, alpha=0.6, width=bar_width, edgecolor='black', linewidth=0.5, linestyle='--', hatch='\\',
            bottom=y_prob, label='delt')  # 柱形图
    current_ylim = ax2.get_ylim()
    new_upper_limit = current_ylim[1] + 0.1
    ax2.set_ylim(current_ylim[0], new_upper_limit)
    ...
    path = f'D:\\1FromDesktop\\study\\0毕设\\结题\\thepngs0\\_{layername}_\\_{one_tgt}_.png'
    ...
    print(f"\033[32msaved in {path}\033[0m")
...
if __name__ == '__main__':
    ...
    if date == 5.22:
        ...
        mistral = [461, 830, 1, 3, 0, 0, 6, 4, 163, 6, 1, 11, 7, 14, 207, 117, 442, 736, 1544, 4423, 3951, 3477, 3913, 4539, 4163, 4429, 6785, 7532, 9971, 12799, 15356, 19940]
        datas = [llama3, mistral]
        draw_line(labels, datas)
        
    elif date == 5.27:
        layer_llama100 = [17, 25, 18, 24, 18, 17, 3, 22, 17, 17, 17, 17, 17, 7, 2, 17, 17, 2, 1, 3, 17, 26, 26, 15, 19, 3, 26, 24, 2, 27, 17, 3, 17, 19, 19, 2, 18, 17, 10, 17, 27, 2, 25, 25, 27, 17, 17, 17, 14, 23, 26, 2, 1, 17, 7, 23, 17, 16, 9, 3, 17, 13, 3, 9, 17, 2, 25, 17, 1, 13, 12, 3, 6, 17, 2, 22, 17, 26, 17, 17, 16, 9, 21, 18, 3, 18, 17, 20, 2, 28, 17, 1, 17, 25, 7, 3, 2, 18, 11, 17]
        # mu=14.43 sigma=8.097227920714596
        print(len(layer_llama100))
        layer_mistral100 = [19, 6, 21, 15, 15, 16, 19, 16, 15, 16, 16, 16, 21, 24, 6, 19, 16, 5, 28, 20, 13, 15, 23, 20, 18, 12, 26, 19, 10, 16, 15, 19, 15, 20, 18, 5, 18, 19, 19, 15, 15, 16, 25, 15, 21, 15, 21, 16, 11, 19, 16, 11, 15, 15, 16, 18, 8, 27, 15, 22, 15, 8, 15, 15, 16, 11, 26, 16, 19, 18, 19, 19, 18, 7, 15, 27, 15, 22, 19, 22, 16, 20, 19, 5, 21, 16, 15, 16, 19, 19, 19, 15, 15, 22, 15, 16, 15, 17, 5, 16]
        print(len(layer_mistral100))
        ...
        num = 1301
        for num in [100, 1301, 1919]:
            if num == 100:
                # 图3-5
                name="100"
                d1(layer_llama100, layer_mistral100, name, num)
                plot_histogram(layer_llama100, "llama_"+name)
                plot_histogram(layer_mistral100, "mistral_" + name)
            elif num == 1301:
                # 图3-6
                name="zsre1301"
            # draw(layer_llama,"llama3")
            # draw(layer_mistral,"mistral")
                d1(layer_llama1000, layer_mistral1000, name, num)

                plot_histogram(layer_llama1000, "llama_"+name)
                plot_histogram(layer_mistral1000, "mistral_" + name)
            elif num == 1919:
                # 图3-7
                name="counterfact1919"
                d1(layer_llama_counterfact, layer_mistral_counterfact, name, num)
                plot_histogram(layer_llama_counterfact, "llama_"+name)
                plot_histogram(layer_mistral_counterfact, "mistral_" + name)
    elif date == 5.24:
        subject = "爱因斯坦"
        r = "的专业是"
        txt = subject + r
        ...
        json_paths = [
            "D:\\1FromDesktop\\study\\0毕设\\结题\\jsons\\物理_info.json",
            "D:\\1FromDesktop\\study\\0毕设\\结题\\jsons\\医学_info.json",
            "D:\\1FromDesktop\\study\\0毕设\\结题\\jsons\\5\\物理_info.json",
            "D:\\1FromDesktop\\study\\0毕设\\结题\\jsons\\5\\医学_info.json",
            "D:\\1FromDesktop\\study\\0毕设\\结题\\jsons\\17\\物理_info.json",
            "D:\\1FromDesktop\\study\\0毕设\\结题\\jsons\\17\\医学_info.json",
        ]
        cnt = -1
        for jsonf in json_paths:
            cnt += 1
            if cnt == 0 or cnt == 1:
                layer = 'no'
            elif cnt == 2 or cnt == 3:
            ...
            dir_name, file_name = os.path.split(jsonf)
            # 分割文件名，以获取token
            token, _ = file_name.split("_info")

            print(f"Layer: {layer}")
            print(f"Token: {token}")

            show_info_in_gragh(info_list=data, one_tgt=token, layername=layer,prompt=txt)